...
class ChatApplication:
    ...
    def run(self):
        welcome_message = (
            "Aura: Hey buddy! Welcome to APNA app. I am your personal chatbot. "
            "I'm here to help you de-stress and nurture yourself. How can I assist you today?"
        )
        self._insert_message(welcome_message, "Aura")
    ...
    # Load existing conversation data
        try:
            with open('conversation3.json', 'r') as json_file:
                existing_data = json.load(json_file)
        except FileNotFoundError:
            existing_data = []

    # Append new data to existing data
        existing_data.extend(self.conversation)

    # Save the combined data to the JSON file
        with open('conversation3.json', 'w') as json_file:
